...
object ReproducibleWeatherProducer{
    ...
    val producer = new KafkaProducer[String, WeatherData](props)
		...
		try {
			...
			while (System.in.available == 0) {
				...
				val currentWeatherData = generateWeatherDataFor(currentGeneration, currentDateTime)
				currentWeatherData.foreach(
					currentWeatherData => {
						val (location, weatherData) = currentWeatherData
						val weatherRecord =
				...
				currentWeatherData.foreach(println)
  ...
  def generateWeatherDataFor(currentGeneration: Int,
																currentDateTime: ZonedDateTime): Iterable[(String, WeatherData)] = {
  	for(
      ...
      val (location, tempsInF) = locationMappedToTempInFRanges1
			weatherDataBuilder.setLocation(location)
  	  ...
  	  val currentTempIndex =
			...
			weatherDataBuilder.setTemperature(tempsInF(currentTempIndex))
			weatherDataBuilder.setWeather(weatherTypes((currentDay + index) % weatherTypes.size))
  	  weatherDataBuilder.setHumidity((currentDay + baseCurrentHour + index) % humidities.size)
			weatherDataBuilder.setWindSpeedInMPH(
				windSpeedInMPHs((currentDay + (baseCurrentHour / 12) + index) % windSpeedInMPHs.size))
			weatherDataBuilder.setEpoch(currentDateTime.toEpochSecond)
			weatherDataBuilder.setNewField("NEW")

  	  (location, weatherDataBuilder.build)
